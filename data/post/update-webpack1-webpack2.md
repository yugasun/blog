...
本文更像是一篇 Change Log 介绍，不涉及的 webpack2 使用教程，感兴趣的可以自行谷歌。之所以直接翻译官方文档，因为只有掌握了 Webpack2 之于 webpack1 的区别和修改，才能更好地进行项目升级。
对于具体的项目升级实例，将会在后面的有关 Vue 项目介绍中提及。
Webpack2 对于 Webpack1 相关配置修改如下：
...
现在 [`json-loader`](https://github.com/webpack/json-loader) 已经内置到 Webpack v2 了，所以你不用再配置它了.
...
Webpack2 需要 [ExtractTextPlugin](https://github.com/webpack/extract-text-webpack-plugin) 版本 2 以上才可使用。
...
最好重构你的代码，如果它不兼容 ES2015 模块。如果实在没法重构，你可以使用 `ContextReplacementPlugin` 来提示编译器正确解析。
...
问得好！好吧，严格的讲，它是可能是两个事，都是来配置 webpack loader 的。webpack1 `options` 被称为 `query`，并且是一个可以附加到 loader 名称后的的字符串，更像是一个 功能更加强大的 `query` 字符串 - [greater powers](https://github.com/webpack/loader-utils#parsequery) :
...
在 webpack1 中，`debug` 是用来配置 loader 为 调试模式的。webpack2 中将使用通过 `loader` 的 `options` 来配置。
...
在 webpack1 中，你可以使用[`require.ensure`](https://webpack.js.org/guides/code-splitting-require) 方法来懒加载模块。
...
webpack2 现在支持 任何 JS 对象作为 loader 配置。
...
例如，`style-loader` 生成一个依赖于剩余请求的模块（它会输出 css）
