...
无论是全局或者局部注册组件，它跟上一篇中的指令注册是非常相似的。局部注册组件就是在创建 Vue 实例的时候添加一个 `components` 对象属性，它的键值对就是一个自定义组件，键是组件名，值是创建组建的配置对象参数。当然也可以将组件定义放到单独的文件，然后通过引入的方式，然后添加到 components 属性中，这个在单文件组件中会具体讲到。
